...
lasso_correlations_all_all <- as.data.frame(lasso_correlations_all)
...
load_bootstrap_results <- function(sample, path = './R/results/bootstrap_results/'){
  file_name <- paste(path, 'bootstrap_sample_', sample, sep = '')
  load(file_name)
  return(fit)
}
@
